 ...
 * @width: self explanatory. the row.
 ...
 * A: test cases if we get a 0 or negetive size. return NULL
 * B: we test if the input is too much so we will return null
 * C: we need to free the test memory. don'twant valgrind on us
 * D: we now habve our pointer, be the size of an int pointer
 ...
 * F: loop througth each index now and insert a 0.
